...
int main ( int argc, char *argv[] )
{
  ...
  //---------------------------------------------------------------
  /* tnt-web patch: upstream tested argc > 0 / argc > 1, dereferencing
     argv[1]/argv[2] that may be NULL (argc counts argv[0]) */
  if ( argc > 1)
  ...
  *outdata <<endl;
  ...
  /* tnt-web patch: this xmgr block sat inside the row loop above and reused
     its loop variable i2, so only the FIRST resistance-matrix row was ever
     written to the .out file (the shipped jc.out_save predates the bug). */
  *xmgrRdata << freq;
  for(i2 = 1; i2 < Nc; i2++)
    {
