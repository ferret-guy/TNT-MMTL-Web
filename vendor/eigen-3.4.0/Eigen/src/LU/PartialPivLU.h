...
#ifdef EIGEN_TNT_LU_PROGRESS
        // TNT: tile reporting without changing the blocked pivoting algorithm.
        for (Index column = 0; column < tsize; column += 64) {
          const Index width = (std::min)(Index(64), tsize-column);
          A22.middleCols(column,width).noalias() -= A21 * A12.middleCols(column,width);
          EIGEN_TNT_LU_PROGRESS(rows, cols, k, bs, double(column+width)/tsize);
        }
#else
        A22.noalias() -= A21 * A12;
#endif
