...
#
# f2c -r8 promotes REAL to double; -R preserves the explicit callback ABI.
# Both flags are REQUIRED: nmmtl_find_nu.cpp passes a double(*)(double*) callback
...
OUT="$TNTWEB_ROOT/build/native-f2c-double-q24-v1"
GEN="$OUT/f2c-gen"
mkdir -p "$OUT/obj" "$GEN"

command -v f2c >/dev/null || { echo "f2c not installed (apt install f2c libf2c2-dev)"; exit 1; }

echo "== f2c translation (-r8 -R) of $(echo "$FORTRAN_SOURCES" | wc -l) files"
