...
# function returns promoted to double, matching math_library.h's extern "C" decls.
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/sources.sh"

OUT="$TNTWEB_ROOT/build/native-gfortran-double-q24-v1"
mkdir -p "$OUT/obj"

CXXFLAGS=(-O2 -DFORTRAN_UNDERBARS -DHAVE_GETLOGIN "-I$SRC_DIR" -std=gnu++14 -Wno-write-strings -fpermissive -w)
FFLAGS=(-O2 -std=legacy -fdefault-real-8 -fdefault-double-8 -w)
