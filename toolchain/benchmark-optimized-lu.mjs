...
const rows = [];
for (const mode of ['linpack', 'eigen', 'eigen']) {
  ...
  try {
    ...
    writeFileSync(new URL(`../build/optimized-lu/${mode}-${rows.length}.field`, import.meta.url), fieldText);
    const row = { mode, elapsedMs: performance.now() - started, result, events };
    rows.push(row);
    writeFileSync(new URL('../build/optimized-lu/measurements.json', import.meta.url), JSON.stringify({ hash, rows }, null, 2));
